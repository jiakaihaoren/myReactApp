...
const cards:string[] = navUris.map(navUri => require(`../../images/${navUri}`));
...
export const DetailImg = () => {
    const [gone] = useState<Set<number>>(() => new Set<number>()); // The set flags all the cards that are flicked out
    const [SpringProps, set] = useSprings(cards.length, i => ({ ...to(i), from: from(i) })); // Create a bunch of springs using the helpers above
    ...
    return (
        <div className={styles.root}>
            {SpringProps.map(({ x, y, rot, scale }, i) =>
                <animated.div className={styles.root1} key={i} style={{ transform: interpolate([x, y], (x, y) => `translate3d(${x}px,${y}px,0)`) }}>
                    {/* This is the card itself, we're binding our gesture to it (and inject its index so we know which is which) */}
                    <animated.div className={styles.root2} {...bind(i)} style={{ transform: interpolate([rot, scale], trans), backgroundImage: `url(${cards[i]})` }} />
                </animated.div>
            )}
        </div>
    );
